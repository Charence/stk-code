...
namespace irr
{
	...
	bool CIrrDeviceWin32::run()
	{
		...
		if (input_manager != NULL && input_manager->isInMode(InputManager::INGAME))
			BsnMessages();
		if (!Close)
			resizeIfNecessary();
...
#define PRODUCT_ULTIMATE_E	0x00000047
	...
	static bool alreadyUp_braking = true;
	...
	void CIrrDeviceWin32::BsnMessages()
	{
		...
		if (bsn_control)
		{
			...
			{
				message = WM_KEYDOWN;
				alreadyUp_braking = false;
				wParam = KEY_DOWN;
				WndProc(HWnd, message, wParam, lParam);
			}
			else if (!alreadyUp_braking)
			{
				message = WM_KEYUP;
				wParam = KEY_DOWN;
				alreadyUp_braking = true;
				WndProc(HWnd, message, wParam, lParam);				
			}
			if (paddling)
			{
				message = WM_KEYDOWN;
				wParam = KEY_UP;
				alreadyUp_paddling = false;				
				WndProc(HWnd, message, wParam, lParam);
			}
			else if (!alreadyUp_paddling)
			{
				message = WM_KEYUP;
				wParam = KEY_UP;
				WndProc(HWnd, message, wParam, lParam);
				alreadyUp_paddling = true;
			}
			if (right_turn)
			{
				message = WM_KEYDOWN;
				alreadyUp_right_turn = false;
				wParam = KEY_RIGHT;
				WndProc(HWnd, message, wParam, lParam);
			}
			else if (!alreadyUp_right_turn)
			{
				message = WM_KEYUP;
				wParam = KEY_RIGHT;
				WndProc(HWnd, message, wParam, lParam);
				alreadyUp_right_turn = true;
			}
			if (left_turn)
			{
				message = WM_KEYDOWN;
				alreadyUp_left_turn = false;
				wParam = KEY_LEFT;
				WndProc(HWnd, message, wParam, lParam);
			}
			else if (!alreadyUp_left_turn)
			{
				message = WM_KEYUP;
				wParam = KEY_LEFT;
				WndProc(HWnd, message, wParam, lParam);
